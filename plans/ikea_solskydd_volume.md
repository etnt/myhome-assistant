...
Intercept volume changes from an LG TV remote and relay them over Bluetooth
Classic (AVRCP) to an IKEA Solskydd speaker connected via optical (S/PDIF)
cable. The ESP32 acts as a bridge: it monitors the TV remote's IR volume
commands and translates them into AVRCP volume commands sent to the speaker.
...
```
 ┌──────────┐              ┌──────────┐   Optical    ┌────────────────┐
 │ LG TV    │ ◄── IR ────  │ LG Remote│   (audio)    │ IKEA Solskydd  │
 │ (webOS)  │ ────────────────────────────────────► │ (BT speaker)   │
 └──────────┘              └─────┬────┘              └───────▲────────┘
                                 │ IR                        │
                                 │                           │ Bluetooth Classic
 ...
 │                                                           │
 │  ┌─────────────────┐     ┌───────────────────────────┐    │
 │  │ IR Receiver     │────►│ AVRCP Volume Controller   │    │
 │  │ (NEC decode)    │     │ (BT Classic A2DP Source)  │    │
 │  └─────────────────┘     └───────────────────────────┘    │
 └───────────────────────────────────────────────────────────┘
```
...
### Phase 1: Validate Bluetooth Behavior

**Goal:** Confirm how the Solskydd handles simultaneous optical + BT connections.

1. Pair a phone to the Solskydd via standard Bluetooth.
2. While optical audio is playing from the TV, connect the phone.
3. Observe: does the speaker keep playing optical audio, or switch to BT?
4. If it stays on optical: use the phone's volume buttons to confirm AVRCP
   volume control works on the optical audio stream.
5. If it switches to BT: test whether disconnecting BT returns to optical,
   and evaluate the "route audio through ESP32" fallback.
6. Optionally: use nRF Connect to check if there are any BLE GATT services
   (VCS) exposed in parallel with the classic BT connection.

**Deliverable:** Confirmed behavior documented in this plan; decision on
...
### Phase 3: Solskydd A2DP/AVRCP Driver (Erlang + NIF)

**Module:** `myhome_solskydd`

A gen_server that maintains a Bluetooth Classic connection to the Solskydd
speaker and sends AVRCP volume commands.

**NIF extension** (in `nifs/bt_classic/`):
- Initialize ESP-IDF Bluetooth Classic stack (Bluedroid or ESP-A2DP).
- Act as A2DP Source, connect to speaker by name/address.
- Expose AVRCP volume control: `set_volume(0..127)`, `volume_up()`, `volume_down()`.
- Send silent audio frames if the speaker requires an active A2DP stream
  (may be needed to keep the connection alive without routing real audio).
- Report connection state changes to Erlang.
...
%% State:
%% - BT Classic connection state (disconnected | connecting | connected)
%% - Current volume level (0..127, internal tracking)
%% - Speaker BT name / MAC address
%% - Reconnection logic (same pattern as myhome_hue_ble)
```

Features:
- Connect to speaker on startup (by name or stored MAC address).
- Maintain internal volume level; send absolute volume via AVRCP on change.
- Expose `volume_up/1`, `volume_down/1`, `set_volume/2`, `mute/1`.
- Handle reconnection on BT disconnect.
- Rate-limit rapid volume changes (max ~10/sec).
- If optical+BT coexistence works: send silence or no audio stream.
- If optical+BT does NOT coexist: optionally route audio through ESP32
  (requires additional analog input hardware — Phase 7).

**Note:** This requires the standard ESP32 (not ESP32-S3) or confirming
that ESP32-S3 supports Bluetooth Classic. Standard ESP32 supports both
BT Classic and BLE; ESP32-S3 supports only BLE. If using ESP32-S3, a
fallback to BLE VCS (if the speaker exposes it) would be needed.
...
| `bt_classic` (NIF) | C | A2DP Source + AVRCP via Bluedroid |
| `myhome_solskydd` | Erlang | BT Classic connection + AVRCP volume commands |
...
- The Solskydd accepts standard Bluetooth connections from a phone without any
  special app (just the OS Bluetooth settings).
- It uses **Bluetooth Classic** (A2DP Sink + AVRCP), not BLE.
- Volume control is via **AVRCP** absolute volume or pass-through commands.
- Standard SSP pairing — no PIN, no proprietary app-level auth required.
- ESP32 must act as **A2DP Source** to connect (similar to a phone).

## Open Questions

1. **Optical + BT coexistence:** Does the speaker continue playing optical audio
   when a BT source connects? Or does it switch to BT input?
   → **Critical — resolve first in Phase 1.**
2. **ESP32 variant:** Standard ESP32 supports BT Classic; ESP32-S3 does NOT.
   Need to use standard ESP32, or confirm if a BLE fallback (VCS) exists.
3. **Volume granularity:** How many discrete AVRCP volume steps does the speaker
   actually respond to? (Standard is 0–127.)
...
6. **Silent A2DP stream:** If the speaker requires an active audio stream to
   keep the AVRCP connection alive, can we send silence without audio artifacts?

## References

- [ESP32-A2DP Library (Phil Schatzmann)](https://github.com/pschatzmann/ESP32-A2DP) — A2DP Source/Sink + AVRCP for ESP32
- [ESP-IDF Bluetooth Classic API](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/bluetooth/classic_bt.html)
- [AVRCP Specification](https://www.bluetooth.com/specifications/specs/a-v-remote-control-profile-1-6-2/)
- [ESP-IDF RMT (Remote Control) Driver](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/rmt.html)
